...
import { Cvp_DashboardCoreAdd_CreateCoreColdtagMutationVariables } from '@/stores/graphql/generated.ts'
...
type DashboardCoreAddPageTemplateProps = {
  title: string
  caption?: string
  loading: boolean
...
const DashboardCoreAddPage: React.FunctionComponent = () => {
  const [, { catchError }] = useErrorState()
  const gqlClient = useGraphQLClient()
  const router = useRouter()

  const [state, setState] = React.useState<DashboardCoreAddPageState>({
    loading: false,
    uploading: false,
    formValues: {},
    formErrors: {},
  ...
  const setFormValues = <T extends keyof CoreFormValues>(
    label: T,
    value: CoreFormValues[T]
  ) => {
    setState((state) => ({
      ...state,
      formValues: {
  ...
  const onContinue = React.useCallback(async () => {
    ...
    try {
      const formErrors: Partial<Record<keyof CoreFormValues, string>> = {
      ...
      const {
        createCoreColdtag: { id },
      } = await gqlClient.CVP_DashboardCoreAdd_CreateCoreColdtag({
        macAddress: formValues.macAddress!,
        identifier: formValues.identifier?.length ? formValues.identifier : undefined,
      })

      router.push(`/dashboard/core/${id}/info`)
    } catch (err) {
...
type CoreFormValues = {
  macAddress?: Cvp_DashboardCoreAdd_CreateCoreColdtagMutationVariables['macAddress']
  identifier?: Cvp_DashboardCoreAdd_CreateCoreColdtagMutationVariables['identifier']
}

type DashboardCoreAddPageState = {
  loading: boolean
  uploading: boolean
  formValues: CoreFormValues
  formErrors: Partial<Record<keyof CoreFormValues, string>>
}

export default DashboardCoreAddPage
