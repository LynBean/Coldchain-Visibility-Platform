...
const RouteCycleAlertEventChart: React.FunctionComponent<{
  events: (
    | Cvp_DashboardCycleReport_DisplayRouteCycleByIdQuery['displayRouteCycle']['byId']['alertTemperatureEvents'][number]
    | Cvp_DashboardCycleReport_DisplayRouteCycleByIdQuery['displayRouteCycle']['byId']['alertHumidityEvents'][number]
  )[]
}> = ({ events }) => {
  type Payload = {
    date: number
        ...
        .map((event, index, arr) => {
          ...
          const value = (() => {
            switch (event.__typename) {
              case 'RouteCycleAlertTemperatureEvent':
                return event.temperature

              case 'RouteCycleAlertHumidityEvent':
          ...
          return {
            date: currentTime,
            type:
              event.__typename === 'RouteCycleAlertTemperatureEvent'
                ? 'temperature'
                : 'humidity',
            temperature:
              event.__typename === 'RouteCycleAlertTemperatureEvent'
                ...
                : undefined,
            humidity:
              event.__typename === 'RouteCycleAlertHumidityEvent'
                ? clampedValue
                : undefined,
      ...
      <Card className="pt-0">
        <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
          <div className="grid flex-1 gap-1">
            ...
            <CardDescription>Showing telemetry alert records of all time</CardDescription>
          </div>
        </CardHeader>
          ...
          >
            ...
            >
              ...
              <Scatter
                ...
                shape="circle"
              />
              <Scatter dataKey="humidity" fill="var(--color-humidity)" shape="triangle" />
        ...
        >
          <div className={tw`grid grid-cols-2 gap-2 py-4`}>
            {(
              [
                (() => {
                  switch (dialogState.current.type) {
                    case 'temperature':
                      return {
                        ...
                        title: 'Temperature',
                        description: (
                          <DialogDescription>{`${dialogState.current.value?.toLocaleString()} °C`}</DialogDescription>
                        ),
                      }
                    ...
                    case 'humidity':
                      return {
                        ...
                        description: (
                          <DialogDescription>
                            {dialogState.current.value?.toLocaleString()}
                          </DialogDescription>
                        ),
                      }
                  }
...
export default RouteCycleAlertEventChart
